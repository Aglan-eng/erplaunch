...
const schema: QuestionnaireSchema = {
  ...
  flows: [
    ...
    // Cross-platform pack — mirrored verbatim in adaptor-netsuite.
    buildTestingFlow(),
    // Pack U — TRAINING flow renders AFTER TESTING (Phase 6 follows
    // Phase 5 in the lifecycle). Same cross-platform pattern.
    buildTrainingFlow(),
    // Pack V — CUTOVER flow renders AFTER TRAINING (Phase 7 follows
    // Phase 6). Cross-platform pack — mirrored verbatim in
    // adaptor-netsuite. Reuses cutoverStyle / cutoverWindowHours /
...
// ─── Pack U — TRAINING flow (CROSS-PLATFORM, mirrored in adaptor-netsuite) ───
//
...
// ─── Pack V — CUTOVER flow (CROSS-PLATFORM, mirrored in adaptor-netsuite) ───
//
// Cutover team, dry runs, go/no-go criteria, rollback triggers, comms.
// Same 8 questions in both adaptors. Reuses cutoverStyle /
// cutoverWindowHours / preFreezeDays from the existing odoo.migration
// flow.
//
// Sources:
//   - ITIL change-management cutover patterns.
//   - Standard ERP cutover runbooks (SuiteSuccess, SAP Activate, Odoo
//     Implementation Methodology cutover guide).
//   - ASTM E2842 — relevant for regulated / life-sciences engagements.
        ...
        questions: [
          {
            ...
            required: false,
            label:
              "Go/No-Go criteria (one per line — '<criterion>: <pass threshold>'; e.g., 'Migration tie-out: 100% TB match across all entities', 'Smoke test pass rate: 100% of P0 scenarios green', 'No Critical defects open: zero', 'Performance benchmarks: all met under simulated peak load')",
          },
          {
