...
function buildTestCases(answers: Record<string, any>): TestCase[] {
  ...
  if (answers['r2r.entities.multiEntity'] === true) {
    tc('R2R', 'Create a transaction in a child subsidiary', 'Transaction is booked in the subsidiary and consolidated into the parent automatically');
  }
  ...
  if (answers['r2r.journalEntries.intercompanyJE'] === true) {
    tc('R2R', 'Create an intercompany journal entry', 'Elimination entry is auto-generated; debit and credit appear in respective subsidiaries');
  }
...
<body>
  <div class="page">
    <div class="header">
      ...
      <p>${data.clientName} &mdash; NetSuite Implementation</p>
      <div class="badges">
    ...
    ${body}
    <div class="footer">Generated by Ofoq NetSuite Accelerator &copy; ${new Date().getFullYear()}</div>
  </div>
</body>
</html>`;
}
